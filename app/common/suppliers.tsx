...
import { fetchSupplierList, type SupplierDetail } from '@/services/suppliers';
...
function SupplierCard({ item, onOpen }: { item: SupplierDetail; onOpen: (name: string) => void }) {
  ...
  const supplierLabel = item.displayName || item.supplierName || item.name;
      ...
      <View style={styles.tagRow}>
        ...
        <View style={[styles.tag, { backgroundColor: muted }]}> 
          <ThemedText style={[styles.tagText, { color: tintColor }]} type="defaultSemiBold">
            {item.supplierGroup || '未分组'}
          </ThemedText>
        </View>
        {item.defaultCurrency ? (
          <>
            <ThemedText style={styles.tagLabel}>币种</ThemedText>
...
export default function SuppliersScreen() {
  ...
  const [items, setItems] = useState<SupplierDetail[]>([]);
  ...
  const loadItems = useCallback(
    async (refresh = false) => {
      try {
        ...
        showError(error instanceof Error ? error.message : '加载供应商失败');
      } finally {
  ...
  return (
    <AppShell compactHeader contentCard={false} description="维护供应商主数据、默认联系人与默认地址，供采购下单、收货、开票和付款流程复用。" title="供应商管理">
      <ScrollView
        ...
        <View style={[styles.heroCard, { backgroundColor: surface, borderColor }]}> 
          ...
          <View style={styles.heroTopRow}>
            <View style={styles.heroCopy}>
              <ThemedText style={styles.heroEyebrow}>SUPPLIER HUB</ThemedText>
              <ThemedText style={styles.heroTitle} type="title">
                供应商中心
              </ThemedText>
              <ThemedText style={styles.heroDescription}>
                用一处入口维护供应商主数据、联系人、地址和结算币种，减少采购链路里的重复录入。
              </ThemedText>
            </View>
            <Pressable onPress={() => router.push('/common/supplier/create')} style={[styles.primaryCta, { backgroundColor: tintColor }]}>
              <IconSymbol color="#FFFFFF" name="building.2.fill" size={16} />
        ...
        <View style={[styles.filterCard, { backgroundColor: surface, borderColor }]}> 
          <View style={styles.filterHeader}>
            <View>
              ...
              <ThemedText style={styles.filterHint}>支持按供应商名称、编码、手机或邮箱快速定位。</ThemedText>
            </View>
          ...
          {items.length ? (
            items.map((item) => (
              <SupplierCard
                item={item}
                key={item.name}
                onOpen={(name) => router.push({ pathname: '/common/supplier/[supplierName]', params: { supplierName: name } })}
              />
            ))
          ) : (
            <View style={[styles.emptyCard, { backgroundColor: surface, borderColor }]}> 
              <View style={styles.emptyIconWrap}>
                <IconSymbol color={tintColor} name="building.2.fill" size={24} />
              </View>
              <ThemedText style={styles.emptyTitle} type="defaultSemiBold">
                没有匹配的供应商
              </ThemedText>
              <ThemedText style={styles.emptyHint}>换个关键词试试，或者直接新增一条供应商主数据。</ThemedText>
            </View>
          )}
        </View>
      </ScrollView>
