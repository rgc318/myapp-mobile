...
export default function SalesOrderPreviewScreen() {
  ...
  const params = useLocalSearchParams<{ salesOrder?: string }>();
  const orderName = typeof params.salesOrder === 'string' ? params.salesOrder.trim() : '';
  ...
  useEffect(() => {
    ...
    router.replace({
      pathname: '/sales/order/pdf-viewer',
      params: {
